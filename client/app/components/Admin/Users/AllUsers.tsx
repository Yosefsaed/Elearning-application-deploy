...
const AllUsers: FC<Props> = ({ isTeam }) => {
    ...
    const { isLoading, data, refetch } = useGetAllUsersQuery({}, { refetchOnMountOrArgChange: true }); // clear the cookies
    const [deleteUser, { isSuccess: deleteSuccess, error: deleteError }] = useDeleteUserMutation()
    useEffect(() => {
        if (updateError) {
            if ("data" in updateError) {
                const errorMessage = updateError as any
                toast.error(errorMessage.data.massage)
            }
        }
        if (isSuccess) {
            refetch()
                    ...
                    <Box m="20px">
                        {isTeam && (
                            <div className="w-full flex justify-end ">
                                <div className={`${styles.button} !w-[200px] dark:bg-[#57c7a3] dark:border !h-[35px] dark:border-[#ffffff67] rounded-[10px]`} onClick={() => setActive(!active)}>
                                    Add New Member
                                </div>
                            </div>
                        )}
                                ...
                                <Modal open={active} onClose={() => setActive(!active)} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                                    <Box className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w[450px] bg-white dark:bg-slate-900 rounded-[8px] shadow p-4 outline-none">
                                        ...
                                        <div className="mt-4">
                                            ...
                                            <select name="" id="" className={`${styles.input} !mt-6 dark:text-white text-black`} value={role} onChange={(e) => setRole(e.target.value)}>
