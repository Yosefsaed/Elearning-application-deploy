...
const CourseContentList: FC<Props> = (props) => {
    ...
    const videoSections: string[] = Array.from(new Set(props.data?.map((item: any) => item.videoSection)));
                            ...
                            isSectionVisible && (
                                <div className="w-full">
                                    {sectionVideos.map((item: any, index: number) => {
                                        ...
                                        return (
                                            <div className={`w-full cursor-pointer transition-all p-2`} key={item._id} onClick={() => props.isDemo ? null : props?.setActiveVideo(videoIndex)}>
                                                <div className="flex items-start">
                                                    <div>
                                                        <MdOutlineOndemandVideo size={25} className="mr-2 text-black dark:text-white" color="#1cdada" />
                                                    </div>
